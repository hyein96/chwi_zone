...
router.get("/", function (req, res) {
  ...
  pool.getConnection(function (err, connection) {
    ...
    let get_sector = connection.query(get_sector_query, function (
      err,
      rows
    ) {
      if (err) {
...
function get_company(sector_id){
    ...
    pool.getConnection(function (err, connection) {
    let get_company_info = connection.query(get_sector_name_query+get_company_query, function (
        err,
        rows,fields
      ) {
        if (err) {
          ...
          if (rows.length == 0) {
            ...
            company.sector_name = result[0][0];
